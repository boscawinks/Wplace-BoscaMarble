  ...
  */
    ...
    window.addEventListener('message', async (event) => {
      ...
      switch (endpointText) {
        ...
        case 'me': // Request to retrieve user data
          ...
          debugLog('Bosca Marble: Paint Data:', this.userPaintData);
  ...
  updateFullChargeInfo(overlay, dataJSON) {
    ...
    if (dataJSON['charges']) {
      ...
      if (window.boscaChargeInterval) {
        clearInterval(window.boscaChargeInterval);
      }

      window.boscaChargeInterval = setInterval(() => {
        this.updateFullChargeDisplay(overlay);
  ...
  updateFullChargeDisplay(overlay) {
    if (!window.boscaChargeData) return;

    const data = window.boscaChargeData;
    ...
    if (data.current >= data.max || remainingMs <= 0) {
      ...
      if (window.boscaChargeInterval) {
        clearInterval(window.boscaChargeInterval);
        window.boscaChargeInterval = null;
      }
      return;
    }
