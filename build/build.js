...
const metaContent = fs.readFileSync('src/BoscaMarble.meta.js', 'utf8');
...
esbuild.build({
  ...
  bundle: true,
  outfile: 'dist/BoscaMarble.user.css',
  minify: true
});
...
const resultEsbuild = await esbuild.build({
  ...
  outfile: 'dist/BoscaMarble.user.js', // The file the bundled code is exported to
...
fs.writeFileSync('dist/BoscaMarble.user.js', resultTerser.code, 'utf8');
...
if (!isGitHub) {
  try {
    importedMapCSS = JSON.parse(fs.readFileSync('dist/BoscaMarble.user.css.map.json', 'utf8'));
  } catch {
...
const mapCSS = mangleSelectors({
  inputPrefix: 'bm-',
  outputPrefix: 'bm-',
  pathJS: 'dist/BoscaMarble.user.js',
  pathCSS: 'dist/BoscaMarble.user.css',
...
if (mapCSS) {
  fs.writeFileSync('dist/BoscaMarble.user.css.map.json', JSON.stringify(mapCSS, null, 2));
}

// Adds the banner
fs.writeFileSync(
  'dist/BoscaMarble.user.js',
  metaContent + fs.readFileSync('dist/BoscaMarble.user.js', 'utf8'),
  'utf8'
);
